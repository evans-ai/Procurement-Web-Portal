<?php

/* @var $this yii\web\View */
use yii\helpers\Html;
use yii\grid\GridView;
use yii\widgets\DetailView;

/* @var $this yii\web\View */
/* @var $model frontend\models\Supplierdata */

$this->title = "RBA Procurement";

$baseUrl = Yii::$app->request->baseUrl;



?>
<!-- BEGIN: Content-->

<!-- Form wizard with number tabs section start -->
<!-- Custom Listgroups start -->
<section id="custom-listgroup">                        
    <div class="row match-height">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title text-uppercase">DETAILS PAGE</h4>
                </div>
                <div class="card-content collapse show">
                    <div class="card-body">
                        <div id="card-header">
                          <h1>WELCOME TO FRC Supplier Portal</h1>
                        </div>
                        <p>
                             On this portal you will be able to view available tender opportunities, apply for prequalification and tenders, view your previous aplications and expired tenders.                               
                        </p>
                        
                                               
                    </div>
                </div>
            </div>
        </div>                        
    </div>
</section>                
<!-- Form wizard with vertical tabs section end -->

<!-- END: Content-->