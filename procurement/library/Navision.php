...
class Navision extends Component
{
    ...
    public function addPrequalifications($credentials, $soapWsdl, $SupplierID){
       /* print '<pre>';
        print_r($Entry); exit;*/
        $client = $this->createClient($credentials, $soapWsdl);
       try {
            $result = $client->CreatePrequalificationRequests(['suppData' => $SupplierID]);
            return $result;
        } catch (\SoapFault $e) {
            return $e->getMessage();
        }
    ...
    public function FinancialResponseLines($credentials, $soapWsdl, $Entry){//Approve any requests
        $client = $this->createClient($credentials, $soapWsdl);
         try {
             $result = $client->ApproveEntry($Entry);
             return $result;
   ...
   public function QuotationsOnPortalCard($credentials, $soapWsdl, $Entry){//Approve any requests
       $client = $this->createClient($credentials, $soapWsdl);
        try {
            $result = $client->ApproveEntry($Entry);
            return $result;
...
//Approval for recruitment
   ...
   public function ResponseLines($credentials, $soapWsdl, $Entry)
    {
        //$result =  $client->Create([$EntryID => $Entry]);
        $client = $this->createClient($credentials, $soapWsdl);
        try {
  ...
  //QuotationEvaluationList
 public function QuotationEvaluationList($credentials, $soapWsdl, $Entry){
         $client = $this->createClient($credentials, $soapWsdl);
        try
        {
...
//Approval for store
    ...
    public function TenderEvaluationList ($credentials, $soapWsdl, $Entry)//cancels a submitted leave request
    {
        $client = $this->createClient($credentials, $soapWsdl);
        try
        {
...
//send recall notification
    ...
    //ResponseCard
    public function ResponseCard($credentials, $soapWsdl, $Entry){
        $client = $this->createClient($credentials, $soapWsdl);
        try
        {
            $result = $client->FnSendRecallNotifications($Entry);
    ...
    //SupplierDocuments
    public function SupplierDocuments($credentials, $soapWsdl, $Entry){
        $client = $this->createClient($credentials, $soapWsdl);
        try
        {
            $result = $client->FnSendRecallNotifications($Entry);
    ...
    //QuotationsVisibleOnPortal
    public function QuotationsVisibleOnPortal($credentials, $soapWsdl, $Entry){
         $client = $this->createClient($credentials, $soapWsdl);
        try
        {
            $result = $client->FnSendRecallNotifications($Entry);
...
//Aprroval for imprest
    ...
    private function createClient($credentials, $soapWsdl)
    {
        ...
        try {
            ...
            stream_wrapper_register('http', '\\procurement\\library\\NTLMStream') or die("Failed to register protocol");
